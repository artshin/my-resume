      ...
      <div class="flex items-start justify-center pt-[20vh] px-4">
        <div class="w-full max-w-lg bg-white dark:bg-gray-800 rounded-xl shadow-2xl overflow-hidden">
          <div class="flex items-center gap-3 px-4 py-3 border-b border-gray-200 dark:border-gray-700">
            ...
            />
            <kbd class="hidden sm:inline-block px-2 py-1 text-xs font-medium text-gray-500 bg-gray-100 dark:bg-gray-700 rounded">ESC</kbd>
          </div>
    ...
    <ul role="listbox" class="py-2">
      ${results.map((result, index) => `
        <li
          ...
          class="search-result px-4 py-3 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 focus:bg-gray-100 dark:focus:bg-gray-700 ${index === 0 ? 'bg-gray-50 dark:bg-gray-700/50' : ''}"
          tabindex="0"
        >
...
function updateSelection(newIndex: number): void {
  ...
  results.forEach((el, i) => {
    el.classList.toggle('bg-gray-50', i === selectedIndex);
    el.classList.toggle('dark:bg-gray-700/50', i === selectedIndex);
  });
